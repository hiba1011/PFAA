...
public class EmploiDuTempsService {
    ...
    public List<Emploi_de_temps> getEmploiDuTempsParClasse(String classe) {
        ...
        String sql = "SELECT s.id, DAYNAME(s.date_debut) AS jour_semaine, TIME(s.date_debut) AS heure_debut, TIME(s.date_fin) AS heure_fin, " +
                "c.intitule AS matiere, sa.numero AS salle, CONCAT(e.prenom, ' ', e.nom) AS enseignant, ? AS groupe " +
                "FROM sessions s " +
                "JOIN cours c ON s.cours_id = c.id " +
                "JOIN enseignants e ON c.enseignant_id = e.id " +
                "JOIN salles sa ON s.salle_id = sa.id " +
                "JOIN enseignant_groupes eg ON eg.enseignant_id = e.id " +
                "WHERE eg.groupe = ?";
